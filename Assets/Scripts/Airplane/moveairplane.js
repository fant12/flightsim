...
var pseudogravitation:float=-0.3; //Abtrieb für Geländefahrten|| downlift for driving through landscape

var rightleftsoft:float=0.0; //Variable für sanften Kurvenflug || Variable for soft curveflight
var rightleftsoftabs:float=0.0; // Positive rightleftsoft Variable 

var divesalto:float =0.0; //Blockt den Vorwärtssalto || blocks the forward salto
...
//-------------------------------------------------------Game over ------------------------------------------------------------------------		
	...
	transform.position = Vector3(0, 1.67f, 0);
	...
	if (Input.GetKey("f2")){
		...
		//Wir müssen alle wichtigen static Values zurücksetzen. Sie sind global, und behalten ihre Werte über die Level bei
		speed=0;
		gameover=0;
		rigidbody.useGravity = false;
...
//------------------------#####     Maincode fliegen || Maincode flying      #####--------------------------------------------------------
	...
	//Hoch Runter, limitiert auf eine Minimalgeschwindigkeit || Up Down, limited to a minimum speed
		...
		//Spezialfall sturzflug über 90 grad || Special case dive above 90 degrees
		if ((Input.GetAxis("Vertical")>0)&&((speed>595))){
	...
	//------------------------------------------------ Neigungskalkulationen / Pitch and Tilt calculations ------------------------------------------
		...
		//Soft rotation Kalkulation  ----Dies verhindert dass das Flugzeug nach links fliegt während es noch nach Rechts geneigt ist
		...
		//Präzisionsproblem rightleftsoft auf Null || Precisionproblem rightleftsoft to zero
	...
	//----------------------------Geschwindigkeit Fahren und Fliegen / Speed driving and flying ----------------------------------------------------------------
		...
		if (speed<0) speed=0; //floatingpoint calculations makes a fix necessary so that speed cannot be below zero
											//Floatingpoint Kalkulation macht ein Fix nötig damit Speed nicht unter Null sein kann
		...
		//Wenn in der Luft weder Gasgeben noch Abbremsen gedrückt wird soll unser Flugzeug auf eine neutrale Geschwindigkeit gehen. 
		//Mit dieser Geschwindigkeit soll es auch neutral in der Höhe bleiben. Drüber soll es steigen, drunter soll es sinken. 
		//Auf diesem Wege lässt sich dann abheben und landen
	...
	//Special case landschaft überfahren. Wir benötigen sowas wie Pseudo Gravitation. 
	...
	//Fahren auf Grund geht bis Speed 600. Fünf Punkte Sicherheit || ground driving is up to Speed 600. Five points security
	if (speed <595){
	//Nase runter beim Hügelrunterfahren
	if ((sensorfront.sensorfront ==0)&&(sensorrear.sensorrear ==1)) transform.Rotate(Time.deltaTime*20,0,0);
	//Nase hoch beim Hügelrunterfahren
	if ((sensorfront.sensorfront ==1)&&(sensorrear.sensorrear ==0)) transform.Rotate(Time.deltaTime*-20,0,0);
	//Nase hoch beim Hügelanfahren
	if (sensorfrontup.sensorfrontup ==1) transform.Rotate(Time.deltaTime*-20,0,0);
	//Pseudoschwerkraft. Könnte man naürlich noch verfeinern indem man den Betrag erhöht. 
	if (groundtrigger.triggered==0) transform.Translate(0,pseudogravitation*Time.deltaTime/10.0,0);
	}
	
	//--------------------------------------------- Debug ---------------------------------------------------------------------------------
	//Mit Key 1 über den Buchstaben kann man sich in die Höhe von 200 katapultieren. Mit Geschwindigkeit 700. Zum debuggen.
	...
	//Hier wird das Flugzeug zur anderen Seite des Levels transportiert wenn es dem Rand zu nahe kommt. Damit ihr nicht unter die Landschaft fliegen könnt
	...
	//Hier wird die Höhe limitiert || Here i limit the height
...
//Wenn unser Flugzeug in der Luft ist (groundtrigger.triggered=0), und mit was anderem als mit seinen Rädern (das groundtrigger Primitive) 
//den Boden berührt wird das als Crash gewertet. 
//Wir müssen in diesem Fall die Geschwindigkeit zu einer Kraft machen damit wir das Flugzeug kollidieren lassen können
